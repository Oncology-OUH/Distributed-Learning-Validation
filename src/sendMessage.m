function [itr,model,central_message,mpi]=sendMessage(mpi,model,itr,ClientIndex,var)
...
while retry < 5 && ~success
    try
        ...
        retry=retry+1;
        disp(['Encountered an exception relating to function call "Client_Receive_from_MPI()" for client: ' num2str(ClientIndex) '. Attempt: ' num2str(retry)])
        disp(err.message);
...
if ~success
    error(['Error after ' num2str(retry) ' attempts recovering from exceptions thrown from "Client_Receive_from_MPI()" for client: ' num2str(ClientIndex)])
end
